...
def _new_board():
    ...
    Returns:
        7x7 tuple of ints
    """
    a = [tuple([0 for i in range(6)]) for j in range(6)]
    return tuple(a)
...
def has_winner(board_state):
    ...
    # check diagonals
    if _has_7_in_a_line([board_state[i][i] for i in range(6)]):
        return -board_state[0][0]
    if _has_7_in_a_line([board_state[5 - i][i] for i in range(6)]):
        return -board_state[0][5]
